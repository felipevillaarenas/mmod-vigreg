...
class MultiModeTrainDataTransform:
    """ Video Transforms for VICReg implemented as in paper's author repository."""
    ...
    def __call__(self, sample):
        # Chunk number
        chunks = int(self.args.total_clip_duration / self.args.subsample_clip_duration)
        ...
        # Applying transforms  
        sample['video'] = self.video_transform(videos[idx]), self.video_transform(videos[idx_prime])
        sample['audio'] = self.audio_transform(audios[idx]), self.audio_transform(audios[idx_prime])
        return sample
